...
i18nTools.createInfoObjectUpdater = function (infoObject, allKeysForLocale, localeId, defaultLocaleId) {
    ...
    return function (options, key, value) {
        ...
        if (isDefaultLocaleId && typeof options.defaultValue !== 'undefined') {
            ...
            if (key in firstSeenDefaultValueByKey && firstSeenDefaultValueByKey[key] !== options.defaultValue) {
...
i18nTools.eachTrInAst = function (ast, lambda) {
    ...
    while (q.length) {
        ...
        if (node[0] === 'call' && node[1][0] === 'call' && node[1][1][0] === 'name' && node[1][1][1] === 'TRPAT') {
            if (lambda({type: 'callTRPAT', key: node[1][2][0][1], node: node, defaultValue: node[1][2][1] && uglifyAst.astToObj(node[1][2][1])}) === false) {
                return;
            }
            ...
            if (lambda({type: node[1][1], key: node[2][0][1], node: node, defaultValue: node[2][1] && uglifyAst.astToObj(node[2][1])}) === false) {
                return;
            }
        }
