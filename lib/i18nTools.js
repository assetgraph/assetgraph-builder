...
i18nTools.eachI18nTagInHtmlDocument = function (document, lambda, nestedTemplateLambda) {
    ...
    while (queue.length) {
        ...
        if (parentNode && node.nodeType === node.ELEMENT_NODE) {
...
i18nTools.createI18nTagReplacer = function (options) {
    ...
    return function i18nTagReplacer(options) {
        ...
        if (localeId && defaultLocaleId && localeId === defaultLocaleId && typeof value !== 'undefined' && typeof options.defaultValue !== 'undefined' && !_.isEqual(options.defaultValue, value) && defaultValueMismatchesByKey) {
...
i18nTools.eachTrInAst = function (ast, lambda) {
    ...
    while (q.length) {
        ...
        if (node[0] === 'call' && node[1][0] === 'call' && node[1][1][0] === 'name' && node[1][1][1] === 'TRPAT') {
            if (lambda({type: 'callTRPAT', key: node[1][2][0][1], node: node, defaultValue: node[1][2][1]}) === false) {
                return;
            }
        } else if (node[0] === 'call' && node[1][0] === 'name' && (node[1][1] === 'TR' || node[1][1] === 'TRPAT')) {
            if (node[2].length === 0 || !Array.isArray(node[2][0]) || node[2][0][0] !== 'string') {
                console.warn("Invalid " + node[1][1] + " syntax: " + uglifyJs.uglify.gen_code(node));
            }
            if (lambda({type: node[1][1], key: node[2][0][1], node: node, defaultValue: node[2][1]}) === false) {
                return;
            }
        }
...
i18nTools.findOccurrences = function (assetGraph, initialAssets) {
    ...
    initialAssets.forEach(function (htmlAsset) {
        var assets = assetGraph.collectAssetsPostOrder(htmlAsset, {type: ['HtmlScript', 'HtmlRequireJsMain', 'JavaScriptInclude', 'JavaScriptAmdDefine', 'JavaScriptAmdRequire', 'JavaScriptTrHtml', 'JavaScriptGetText']}).forEach(function (asset) {
