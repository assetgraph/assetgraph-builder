...
function extractKeyAndDefaultValueFromCallNode(callNode) {
    var argumentAsts = callNode[2];

    if (argumentAsts.length === 0) {
        console.warn("Invalid " + callNode[1][1] + " syntax: " + uglifyJs.uglify.gen_code(callNode));
    } else {
        var keyNameAst = argumentAsts.length > 0 && uglifyAst.foldConstant(argumentAsts[0]),
            ...
            keyAndDefaultValue = {};
        if (keyNameAst && keyNameAst[0] === 'string') {
            keyAndDefaultValue.key = keyNameAst[1];
            if (defaultValueAst) {
                try {
                    keyAndDefaultValue.defaultValue = uglifyAst.astToObj(uglifyAst.foldConstant(defaultValueAst));
                } catch (e) {
                    console.warn('i18nTools.eachTrInAst: Invalid ' + callNode[1][1]+ ' default value syntax: ' + uglifyJs.uglify.gen_code(callNode));
                }
            }
            return keyAndDefaultValue;
        } else {
            console.warn('i18nTools.eachTrInAst: Invalid ' + callNode[1][1] + ' key name syntax: ' + uglifyJs.uglify.gen_code(callNode));
        }
    }
}
...
i18nTools.eachTrInAst = function (ast, lambda) {
    ...
    while (q.length) {
        var node = q.pop();
        if (node[0] === 'call' && node[1][0] === 'call' && node[1][1][0] === 'name' && node[1][1][1] === 'TRPAT') {
            var keyAndDefaultValue = extractKeyAndDefaultValueFromCallNode(node[1]);
            if (keyAndDefaultValue) {
                if (lambda(_.extend(keyAndDefaultValue, {type: 'callTRPAT', node: node})) === false) {
                    return;
                }
            }
        } else if (node[0] === 'call' && node[1][0] === 'name' && (node[1][1] === 'TR' || node[1][1] === 'TRPAT')) {
            var keyAndDefaultValue = extractKeyAndDefaultValueFromCallNode(node);
            if (keyAndDefaultValue) {
                if (lambda(_.extend(keyAndDefaultValue, {type: node[1][1], node: node})) === false) {
                    return;
                }
            }
        }
...
i18nTools.findOccurrences = function (assetGraph, initialAssets) {
    ...
    initialAssets.forEach(function (htmlAsset) {
        var assets = assetGraph.collectAssetsPostOrder(htmlAsset, {type: assetGraph.constructor.query.not('HtmlAnchor')}).forEach(function (asset) {
...
i18nTools.getOrCreateI18nAssetForKey = function (assetGraph, key, occurrencesByKey) {
    ...
    if (i18nAssetsWithTheKey.length === 1) {
        ...
        if (existingI18nRelations.length === 0) {
            ...
            var relation = new assetGraph.constructor.relations.JavaScriptInclude({
