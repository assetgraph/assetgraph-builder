...
i18nTools.createInfoObjectUpdater = function (infoObject, allKeysForLocale, localeId, defaultLocaleId) {
    ...
    var localeIdsByMissingKey = (infoObject.localeIdsByMissingKey = infoObject.localeIdsByMissingKey || {}),
        localeIdsByUntranslatedKey = (infoObject.localeIdsByUntranslatedKey = infoObject.localeIdsByUntranslatedKey || {}),
...
i18nTools.formatWarningsAboutLanguageKeys = function (localizationInfoObject, defaultLocaleId) {
    var warnings = [],
        missingKeys = Object.keys(localizationInfoObject.localeIdsByMissingKey || {});
    if (missingKeys.length > 0) {
        warnings.push('The following keys were missing:\n  ' + missingKeys.map(function (missingKey) {
            return missingKey + ' (' + localizationInfoObject.localeIdsByMissingKey[missingKey].join(',') + ')';
        }).join('\n  '));
    }

    var untranslatedKeys = Object.keys(localizationInfoObject.localeIdsByUntranslatedKey || {});
    if (untranslatedKeys.length > 0) {
        warnings.push('The following keys were missing, but were flagged for translation:\n  ' + untranslatedKeys.map(function (untranslatedKey) {
            return untranslatedKey + ' (' + localizationInfoObject.localeIdsByUntranslatedKey[untranslatedKey].join(',') + ')';
        }).join('\n  '));
    }
    ...
    if (defaultValueMismatchKeys.length > 0) {
        warnings.push('The following keys had mismatching default and/or ' + defaultLocaleId + ' values:\n  ' + defaultValueMismatchKeys.map(function (defaultValueMismatchKey) {
    ...
    if (whitespaceWarningKeys.length > 0) {
        warnings.push('The following keys had leading or trailing whitespace:\n  ' + whitespaceWarningKeys.map(function (whitespaceWarningKey) {
