...
function bootstrapperCode() {
    ...
    var documentElement = document && document.documentElement,
        documentElementLang = documentElement && normalizeLocaleId(documentElement.getAttribute('lang'));
    ...
    function findActiveLocaleId() {
        ...
        if (window.SUPPORTEDLOCALEIDS) {
            for (var i = 0 ; i < SUPPORTEDLOCALEIDS.length ; i += 1) {
                isSupportedByLocaleId[normalizeLocaleId(SUPPORTEDLOCALEIDS[i])] = true;
            }
        }
        function isSupportedLocaleId(localeId) {
            // Assume that all locales are supported if window.SUPPORTEDLOCALEIDS isn't defined
            return !window.SUPPORTEDLOCALEIDS || isSupportedByLocaleId[normalizeLocaleId(localeId)];
        }
        ...
        if (window.LOCALECOOKIENAME) {
            ...
            if (matchLocaleCookieValue) {
                var cookieLocaleId = normalizeLocaleId(matchLocaleCookieValue[1]);
        ...
        if (window.SUPPORTEDLOCALEID && SUPPORTEDLOCALEID.length > 0) {
            return SUPPORTEDLOCALEID[0];
        }
    ...
    if (document && document.childNodes && window.TRANSLATE !== false) {
