...
function oneBootstrapperCode() {
    ...
    (function installOneDevelopmentMode() {
        ...
        if (!document.documentElement || document.documentElement.lang && one.localeCookieName) {
            // Make sure that one.localeId is correct in development mode:
            var matchLocaleCookieValue = document.cookie && document.cookie.match(new RegExp("\\b" + one.localeCookieName.replace(/[\.\+\*\{\}\[\]\(\)\?\^\$]/g, '\\$&') + "=([\\w]+)"));
