...
i18nTools.eachI18nTagInHtmlDocument = function (document, lambda) {
    ...
    while (queue.length) {
        ...
        if (parentNode && node.nodeType === node.ELEMENT_NODE && node.hasAttribute('data-i18n')) {
            ...
            if (lambda(node, key, defaultValue, placeHolders) === false) {
                return;
            }
            if (!node.parentNode) {
...
i18nTools.createI18nTagReplacer = function (allKeys, localeId) {
    return function i18nTagReplacer(node, key, defaultValue, placeHolders) {
        var value = allKeys[key] || defaultValue || '[!' + key + '!]',
            removeNode = node.nodeName.toLowerCase() === 'span' && node.attributes.length === 1;
        ...
        i18nTools.tokenizePattern(value).forEach(function (token) {
            ...
            if (token.type === 'text') {
                ...
                nodeToInsert = placeHolders[token.value] || node.ownerDocument.createTextNode('[!{' + token.value + '}!]');
            }
            if (removeNode) {
...
i18nTools.extractAllReachableKeys = function (assetGraph, initialAssets) {
    ...
    initialAssets.forEach(function (initialAsset) {
        assetGraph.collectAssetsPostOrder(initialAsset, {type: ['HtmlScript', 'HtmlRequireJsMain', 'JavaScriptOneInclude']}).filter(function (asset) {
    ...
    initialAssets.forEach(function (htmlAsset) {
        var assets = assetGraph.collectAssetsPostOrder(htmlAsset, {type: ['HtmlScript', 'HtmlRequireJsMain', 'JavaScriptOneInclude', 'JavaScriptAmdDefine', 'JavaScriptAmdRequire', 'JavaScriptOneGetText']}).forEach(function (asset) {
            if (asset.type === 'JavaScript') {
                i18nTools.eachOneTrInAst(asset.parseTree, function (type, key, node, defaultValueAst) {
                ...
                i18nTools.eachI18nTagInHtmlDocument(asset.parseTree, function (node, key, defaultValue, placeHolders) {
                    (oneTrOccurrencesByKey[key] = oneTrOccurrencesByKey[key] || []).push({
                        asset: asset,
                        type: 'i18nTag',
                        node: node,
