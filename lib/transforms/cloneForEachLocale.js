...
module.exports = function (queryObj, localeIds) {
    return function cloneForEachLocale(assetGraph) {
        assetGraph.findAssets(_.extend({type: 'Html'}, queryObj)).forEach(function (originalHtmlAsset) {
            ...
            assetGraph.findRelations({type: 'HtmlScript', from: originalHtmlAsset, to: {url: query.isDefined}, node: {id: query.not('oneBootstrapper')}}).forEach(function (htmlScript) {
                var hasOneTr = false;
            ...
            localeIds.forEach(function (localeId) {
                ...
                var allKeys = i18nTools.extractAllReachableKeysForLocale(assetGraph, localeId, localizedHtml);
                ...
                assetGraph.findRelations({type: 'HtmlScript', from: localizedHtml, node: {id: query.not('oneBootstrapper')}}).forEach(function (htmlScript) {
                    if (htmlScript.to.id in nonInlineJavaScriptsToCloneById) {
                        ...
                        i18nTools.eachOneTrInAst(clonedJavaScript.parseTree, i18nTools.createOneTrReplacer(allKeys, localeId));
