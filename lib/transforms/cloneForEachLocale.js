        ...
        if (asset.incomingRelations.every(function (incomingRelation) {
            ...
            asset.parseTree.walk(new uglifyJs.TreeWalker(function (node, descend) {
                if (!needsLocalization) {
                    if ((node instanceof uglifyJs.AST_SymbolRef && /^(?:LOCALEID|SUPPORTEDLOCALEIDS|DEFAULTLOCALEID)$/.test(node.name) && !isLeftHandSideOfAssignment(this.stack)) ||
...
function followRelationFn(relation) {
    return relation.type !== 'HtmlAnchor' && relation.type !== 'SvgAnchor' && relation.type !== 'JavaScriptSourceUrl' && !isBootstrapperRelation(relation);
}
