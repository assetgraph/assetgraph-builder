...
function getParsedDataSystemJsConditionalsFromRelation(relation) {
    if (relation.type === 'HtmlScript' || relation.type === 'HtmlStyle') {
        var dataCond = relation.node.getAttribute('data-systemjs-conditionals');
        if (dataCond) {
                        ...
                        if (localeValue) {
                            if (localeValue === localeId) {
                                ...
                                if (remainingKeys.length === 0) {
                                    incomingHtmlScriptsAndHtmlStyles[0].node.removeAttribute('data-systemjs-conditionals');
                                } else {
                                    incomingHtmlScriptsAndHtmlStyles[0].node.setAttribute('data-systemjs-conditionals', remainingKeys.map(function (key) {
