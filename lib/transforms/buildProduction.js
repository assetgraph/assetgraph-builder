    ...
    return async function buildProduction(assetGraph) {
        ...
        if (!assetGraph.followRelations) {
            ...
            if (options.recursive) {
                followRelations = { $or: [ { type: { $in: ['HtmlAnchor', 'HtmlMetaRefresh'] } }, { type: { $nin: excludeRelationTypes } } ] };
            } else {
        ...
        await assetGraph.removeRelations({ type: { $in: [ 'SourceMapSource', 'SourceMapFile' ] } }, { unresolved: true });
        await assetGraph.populate();
        ...
        await assetGraph.mergeIdenticalAssets({isLoaded: true, isInline: false, type: { $in: ['JavaScript', 'Css'] }});
        ...
        if (options.sourceMaps) {
            ...
            for (const relation of assetGraph.findRelations({ type: { $regex: /SourceMappingUrl$/ }, hrefType: 'inline' })) {
