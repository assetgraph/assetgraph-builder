...
module.exports = function (options) {
    ...
    return function buildProduction(assetGraph, cb) {
        ...
        assetGraph
            .bundleWebpack()
            .populate({from: {type: 'Html'}, followRelations: {type: 'HtmlScript', crossorigin: false}})
            .bundleSystemJs({
            ...
            .cloneForEachConditionValue({type: 'Html', isInitial: true}, {
                conditions: options.splitConditions
            })
            .if(minify)
                // inline assets have to be minified after cloneForEach*
                // because the isMinified attribute unfortunately isn't retained
                // in the clone:
                .minifyAssets({isLoaded: true, isInline: true})
            .endif()
            ...
            .queue(function omitFunctionCalls(assetGraph) {
                assetGraph.findRelations({type: ['JavaScriptStaticUrl', 'JavaScriptTrHtml'], to: {isLoaded: true}}).forEach(function (relation) {
