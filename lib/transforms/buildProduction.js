...
module.exports = function (options) {
    ...
    return function buildProduction(assetGraph, cb) {
        assetGraph
            ...
            .processImages({isImage: true, url: /\?(?:|.*&)sprite(?:[&=#]|$)/})
            .spriteBackgroundImages()
            // Execute explicit and automatic optimizations for all images, including the generated sprite images:
            .processImages({isImage: true}, {
                pngcrush: options.optimizePngs, // Backwards compatible, could be turned into its own option
                optipng: options.optimizePngs, // Backwards compatible, could be turned into its own option
                pngquant: options.optimizePngs, // Backwards compatible, could be turned into its own option
                jpegtran: options.optimizeJpegs // Backwards compatible, could be turned into its own option
            })
            ...
            .moveAssetsInOrder({isInitial: query.not(true), type: query.not('CacheManifest')}, function (asset, assetGraph) {
                ...
                if (options.cdnRoot && asset.type !== 'Htc' && (asset.isImage || assetGraph.findRelations({to: asset, type: 'StaticUrlMapEntry'}).length === 0)) {
