module.exports = function (options) {
    ...
    var bundleStrategy = options.sharedBundles ? 'sharedBundles' : 'oneBundlePerIncludingAsset';
    ...
    return function buildProduction(assetGraph, cb) {
        ...
        assetGraph
            ...
            .mergeIdenticalAssets({isImage: true, isLoaded: true})
            ...
            // Temporary workaround: Prevent text!foo.css with paths from breaking:
            .removeRelations({type: ['JavaScriptAmdRequire', 'JavaScriptAmdDefine', 'JavaScriptShimRequire', 'JavaScriptRequireJsCommonJsCompatibilityRequire'], to: {type: 'Css'}}, {unresolved: true})
            ...
            .bundleRelations({type: 'HtmlStyle', to: {type: 'Css', isLoaded: true}, node: function (node) {return !node.hasAttribute('nobundle');}}, bundleStrategy)
            .bundleRelations({type: 'HtmlScript', to: {type: 'JavaScript', isLoaded: true}, node: function (node) {return !node.hasAttribute('nobundle');}}, bundleStrategy)
            ...
            .compressJavaScript({type: 'JavaScript', isLoaded: true}, 'uglifyJs', {global_defs: options.defines})
            .inlineRelations({
            ...
            // Prevent moveAssetsInOrder from messing up the module names:
            .removeRelations({type: ['JavaScriptAmdRequire', 'JavaScriptAmdDefine', 'JavaScriptShimRequire', 'JavaScriptRequireJsCommonJsCompatibilityRequire']}, {unresolved: true})
            .if(options.manifest)
