...
module.exports = function (options) {
    ...
    return async function buildProduction(assetGraph) {
        ...
        for (const relation of assetGraph.findRelations({type: {$in: ['HtmlScript', 'HtmlStyle']}})) {
        ...
        if (options.sourceMaps) {
            ...
            for (const relation of assetGraph.findRelations({ type: { $regex: /SourceMappingUrl$/ }, hrefType: 'relative' })) {
                relation.hrefType = 'rootRelative';
            }
            await assetGraph.removeRelations({ type: ['SourceMapSource', 'SourceMapFile'] }, { unresolved: true });
        }
