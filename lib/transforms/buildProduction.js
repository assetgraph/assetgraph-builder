        ...
        assetGraph
            ...
            .moveAssetsInOrder({isLoaded: true, isInline: false, type: query.not('CacheManifest')}, function (asset, assetGraph) {
                if (!asset.isInitial || (asset.type === 'Html' && asset.isFragment)) {
                    ...
                    if (options.cdnRoot && asset.type !== 'Htc' && (asset.type !== 'Html' || options.cdnHtml) && (asset.isImage || assetGraph.findRelations({to: asset, type: 'StaticUrlMapEntry'}).length === 0) || (options.cdnRoot && options.cdnFlash && asset.type === 'Flash')) {
