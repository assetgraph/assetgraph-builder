...
module.exports = function (options) {
    ...
    return function buildProduction(assetGraph, cb) {
        ...
        if (!assetGraph.followRelations) {
            var followRelations;
            var excludeRelationTypes = ['SvgAnchor', 'SourceMapSource', 'SourceMapFile', 'JavaScriptFetch'];
        ...
        assetGraph
            ...
            .processImages({isImage: true, isLoaded: true, url: { $regex: /\?(?:|.*&)sprite(?:[&=#]|$)/ }})
            ...
            .if(options.svgo)
                ...
                .populate({startAssets: {type: 'Svg'}, followRelations: { from: { type: 'Svg' }, crossorigin: false }})
            .endif()
            .removeUnreferencedAssets({isInitial: {$not: true}})
            ...
            .if(!options.noCompress)
                .compressJavaScript({type: 'JavaScript', isLoaded: true}, 'uglifyJs', { sourceMaps: options.sourceMaps, mangleOptions: { reserved: options.reservedNames || [] } })
                // Pick up source map references from the compressed JavaScript:
                .populate(getFollowRelationsQuery({from: {type: 'JavaScript', isLoaded: true}}))
            .endif()
            ...
            .if(options.sourceMaps)
                ...
                .updateRelations({ type: { $regex: /SourceMappingUrl$/ }, hrefType: 'relative' }, { hrefType: 'rootRelative' }, { includeUnresolved: true })
                .removeRelations({ type: [ 'SourceMapSource', 'SourceMapFile' ] }, { unresolved: true })
            .endif()
            ...
            .if(!options.noFileRev)
                .moveAssetsInOrder(moveAssetsInOrderQuery, function (asset, assetGraph) {
                    ...
                    var hasIncomingJavaScriptStaticUrlOrServiceWorkerRelations = assetGraph.findRelations({to: asset, type: ['JavaScriptStaticUrl', 'JavaScriptServiceWorkerRegistration', 'HtmlServiceWorkerRegistration']}).length > 0;
            ...
            .if(options.precacheServiceWorker)
                .addPrecacheServiceWorker({isInitial: true, isLoaded: true, isInline: false, type: 'Html'}, {single: true})
                .queue(function omitFunctionCalls(assetGraph) {
                    assetGraph.findRelations({type: 'JavaScriptStaticUrl', from: { fileName: { $regex: /precache-service-worker\.js$/ } }, to: {isLoaded: true}}).forEach(function (relation) {
                        relation.omitFunctionCall();
                    });
                })
                .if(minify)
            ...
            .if(options.gzip)
                .gzip({isInline: false, isText: true, isLoaded: true, extension: {$nin: ['.tgz', '.gz']}})
            .endif()
            .run(cb);
    };
};
