        ...
        assetGraph
            ...
            .if(!options.noFileRev)
                .moveAssetsInOrder(moveAssetsInOrderQuery, function (asset, assetGraph) {
                    ...
                    var hasIncomingJavaScriptStaticUrlRelations = assetGraph.findRelations({to: asset, type: 'JavaScriptStaticUrl'}).length > 0;
                    if (options.cdnRoot && asset.type !== 'Htc' && asset.extension !== '.jar' && (asset.type !== 'Html' || options.cdnHtml) && (asset.isImage || !hasIncomingJavaScriptStaticUrlRelations) || (options.cdnRoot && options.cdnFlash && asset.type === 'Flash')) {
                        ...
                        assetGraph.findRelations({to: asset}).forEach(function (incomingRelation) {
                            ...
                            } else if ((asset.type === 'SourceMap' || hasIncomingJavaScriptStaticUrlRelations) && /^https?:/.test(options.cdnRoot)) {
