...
module.exports = function (options) {
    ...
    if (options.browsers) {
        browsers = {
            ...
            has(browserOrBrowsers) {
                try {
                    return browsersList(browserOrBrowsers).some(
                        browser => browsers.selected.some(
                            selectedBrowser => selectedBrowser === browser || selectedBrowser.indexOf(browser + ' ') === 0
                        )
                    );
                } catch (e) {
                    // Parse error, try to match it as a browser name (any version) so that
                    // browsers.has('ie') does the expected.
                    return browsers.selected.some(
                        selectedBrowser => selectedBrowser.startsWith(browserOrBrowsers + ' ')
                    );
                }
            },
            ...
            minimum(browser) {
                ...
                for (const selectedBrowser of browsers.selected) {
                    if (selectedBrowser.startsWith(browser + ' ')) {
                        const version = parseFloat(selectedBrowser.substr(browser.length + 1));
    ...
    const bundleStrategyName = options.sharedBundles ? 'sharedBundles' : 'oneBundlePerIncludingAsset';
    const inlineByRelationType = options.inlineByRelationType || {HtmlScript: 4096, HtmlStyle: 4096};
    ...
    return async function buildProduction(assetGraph) {
        ...
        assetGraph.javaScriptSerializationOptions = _.defaults({}, options.javaScriptSerializationOptions, {
        ...
        function getFollowRelationsQuery(followRelationsQuery) {
            if (excludePatterns) {
                return { $and: [ followRelationsQuery, { to: { url: { $not: excludePatterns } } } ] };
            }
        ...
        if (!assetGraph.followRelations) {
            let followRelations;
            const excludeRelationTypes = ['SvgAnchor', 'SourceMapSource', 'SourceMapFile', 'JavaScriptFetch'];
            if (!options.sourceMaps) {
                excludeRelationTypes.push('JavaScriptSourceMappingUrl', 'JavaScriptSourceUrl', 'CssSourceMappingUrl', 'CssSourceUrl');
            }
            if (options.recursive) {
                followRelations = { $or: [ { type: ['HtmlAnchor', 'HtmlMetaRefresh'] }, { type: { $nin: excludeRelationTypes } } ] };
            } else {
        ...
        await assetGraph.populate(getFollowRelationsQuery({from: {type: 'Html'}, followRelations: {type: 'HtmlScript', crossorigin: false}}));
        ...
        await assetGraph.populate(getFollowRelationsQuery({startAssets: {type: 'JavaScript'}}));

        // Rename transpiled Css assets:
        for (const cssAsset of assetGraph.findAssets({isInline: false, type: 'Css', extension: {$not: '.css'}})) {
        ...
        await assetGraph.removeRelations({ type: [ 'SourceMapSource', 'SourceMapFile' ] }, { unresolved: true });
        await assetGraph.populate();
        await assetGraph.populate(getFollowRelationsQuery({startAssets: {type: 'Html'}}));

        // Remove bootstrapper scripts injected by buildDevelopment:
        await assetGraph.removeRelations({type: 'HtmlScript', node: {id: 'bootstrapper'}, from: {type: 'Html'}}, {detach: true, removeOrphan: true});

        await assetGraph.addDataVersionAttributeToHtmlElement({type: 'Html', isInitial: true}, options.version);
        ...
        await assetGraph.processImages({isImage: true, isLoaded: true, url: { $regex: /\?(?:|.*&)sprite(?:[&=#]|$)/ }});
        ...
        await assetGraph.processImages({isImage: true, isLoaded: true}, {autoLossless: options.optimizeImages});

        if (options.svgo) {
            await assetGraph.minifySvgAssetsWithSvgo({isLoaded: true});
            // Won't be necessary with AssetGraph 4:
            await assetGraph.populate({startAssets: {type: 'Svg'}, followRelations: { from: { type: 'Svg' }, crossorigin: false }});
        }
        await assetGraph.removeUnreferencedAssets({isInitial: {$not: true}});
        await assetGraph.convertCssImportsToHtmlStyles();
        await assetGraph.removeDuplicateHtmlStyles({type: 'Html', isInitial: true});
        await assetGraph.mergeIdenticalAssets({isLoaded: true, isInline: false, type: ['JavaScript', 'Css']});

        if (options.browsers) {
            await assetGraph.autoprefixer(options.browsers, { sourceMaps: options.sourceMaps, sourcesContent: options.sourcesContent });
        }

        if (options.addInitialHtmlExtension) {
            // Add initial html extension

            for (const asset of assetGraph.findAssets({isInitial: true, type: 'Html'})) {
                asset.fileName = asset.fileName.replace(/(?=\.|$)/, '.' + String(options.addInitialHtmlExtension).replace(/^\./, ''));
            }
        }

        if (minify) {
            // This is annoying, fix before releasing assetgraph 4 by making Css#minify an async method:
            for (const asset of assetGraph.findAssets({isLoaded: true, isInline: false})) {
                if (asset.minify) {
                    asset.minify();
                } else if (asset.type === 'Css') {
                    await assetGraph.minifyCss(asset);
                }
            }
        }

        await assetGraph.cloneForEachConditionValue({type: 'Html', isInitial: true}, {
            splitConditions: options.splitConditions,
            conditions: options.conditions
        });
        await assetGraph.inlineHtmlTemplates();
        await assetGraph.bundleRelations({type: 'HtmlStyle', to: {type: 'Css', isLoaded: true}, node: function (node) {return !node.hasAttribute('nobundle');}}, {strategyName: bundleStrategyName});
        await assetGraph.splitCssIfIeLimitIsReached({type: 'Css'}, {minimumIeVersion: browsers.minimum('ie')});
        await assetGraph.bundleRelations({type: 'HtmlScript', to: {type: 'JavaScript', isLoaded: true}, node: function (node) {return !node.hasAttribute('nobundle');}}, {strategyName: bundleStrategyName});
        await assetGraph.bundleRelations({type: 'JavaScriptImportScripts', to: {type: 'JavaScript', isLoaded: true}}, {strategyName: bundleStrategyName});
        ...
        await assetGraph.removeNobundleAttribute({type: ['HtmlScript', 'HtmlStyle']});
        ...
        if (inlineByRelationType.CssImage) {
            ...
            await assetGraph.mergeIdenticalAssets({isLoaded: true, isInline: false, type: 'Css'});
        }

        if (options.defines) {
            await assetGraph.replaceSymbolsInJavaScript({type: 'JavaScript', isLoaded: true}, options.defines);
        }

        if (!options.noCompress) {
            await assetGraph.compressJavaScript({type: 'JavaScript', isLoaded: true}, 'uglifyJs', { sourceMaps: options.sourceMaps, mangleOptions: { reserved: options.reservedNames || [] } });
            // Pick up source map references from the compressed JavaScript:
            await assetGraph.populate(getFollowRelationsQuery({from: {type: 'JavaScript', isLoaded: true}}));
        }
        ...
        if (options.pretty) {
            for (const asset of assetGraph.findAssets({type: 'Css', isLoaded: true})) {
                asset.prettyPrint();
            }
                    ...
                    { isInline: true },
                    {
                        incomingRelations: {
                            $where: incomingRelations =>
                                incomingRelations.every(incomingRelation => incomingRelation.type === 'HtmlScript')
                        }
                    }
                ]
            })) {
        ...
        await assetGraph.setAsyncOrDeferOnHtmlScripts({crossorigin: false, to: {isInline: false}}, options.asyncScripts, options.deferScripts);

        // Omit .toString('url') function calls:

        for (const relation of assetGraph.findRelations({type: 'JavaScriptStaticUrl', to: {isLoaded: true}})) {
        ...
        if (assetGraph.canonicalRoot) {
            await assetGraph.updateRelations({
                to: {
                    isLoaded: true,
                    isInline: false
                },
        ...
        if (options.sourceMaps) {
            await assetGraph.serializeSourceMaps({ sourcesContent: options.sourcesContent });
            await assetGraph.updateRelations({ type: 'SourceMapSource' }, { hrefType: 'rootRelative' }, { includeUnresolved: true });
            await assetGraph.updateRelations({ type: { $regex: /SourceMappingUrl$/ }, hrefType: 'relative' }, { hrefType: 'rootRelative' }, { includeUnresolved: true });
            await assetGraph.removeRelations({ type: [ 'SourceMapSource', 'SourceMapFile' ] }, { unresolved: true });
        }
        ...
        if (!options.noFileRev) {
            await assetGraph.moveAssetsInOrder(moveAssetsInOrderQuery, (asset, assetGraph) => {
                ...
                const hasIncomingJavaScriptStaticUrlOrServiceWorkerRelations = assetGraph.findRelations({to: asset, type: ['JavaScriptStaticUrl', 'JavaScriptServiceWorkerRegistration', 'HtmlServiceWorkerRegistration']}).length > 0;
        ...
        if (options.precacheServiceWorker) {
            await assetGraph.addPrecacheServiceWorker({isInitial: true, isLoaded: true, isInline: false, type: 'Html'}, {single: true});

            // Omit .toString('url') function calls in the generated service worker:

            for (const relation of assetGraph.findRelations({type: 'JavaScriptStaticUrl', from: { fileName: { $regex: /precache-service-worker\.js$/ } }, to: {isLoaded: true}})) {
                relation.omitFunctionCall();
            }
        ...
        await assetGraph.reviewContentSecurityPolicy({type: 'Html', isInline: false, isFragment: false, isLoaded: true}, {
            update: options.contentSecurityPolicy,
            level: options.contentSecurityPolicyLevel,
            includePath: options.contentSecurityPolicyLevel >= 2 || !(browsers.has('Safari 8') || browsers.has('Safari 9') || browsers.has('Safari 9.1')) ?
                ['script-src', 'style-src', 'frame-src', 'object-src', 'manifest-src', 'child-src'] :
                false
        });

        await assetGraph.reviewSubResourceIntegrity({type: 'Html', isInline: false, isFragment: false, isLoaded: true}, {update: options.subResourceIntegrity});

        if (options.gzip) {
            await assetGraph.gzip({isInline: false, isText: true, isLoaded: true, extension: {$nin: ['.tgz', '.gz']}});
        }
    };
};
