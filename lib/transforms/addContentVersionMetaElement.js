    ...
    return function addContentVersionMetaElement(assetGraph) {
        assetGraph.findAssets(_.extend({type: 'Html'}, queryObj)).forEach(function (htmlAsset) {
            ...
            if (updateExisting) {
                ...
                for (var i = 0 ; i < metaTags.length ; i += 1) {
                    var metaTag = metaTags[i],
                        httpEquivAttributeValue = metaTag.getAttribute('http-equiv');
                    if (typeof httpEquivAttributeValue === 'string' && httpEquivAttributeValue.toLowerCase() === 'content-version') {
            ...
            if (document.head) {
                ...
                contentVersionMetaElement.setAttribute('content', String(version).replace(/\{0\}/g, ''));
