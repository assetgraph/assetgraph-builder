...
module.exports = (queryObj, options) => {
  ...
  return async function processImages(assetGraph) {
    ...
    const prepareImproQueryString = require('express-processimage/lib/prepareImproQueryString');
    ...
    await promiseMap(
      ...
      async imageAsset => {
        ...
        if (matchQueryString) {
          const improQueryString = prepareImproQueryString(matchQueryString[1]);
          const parsedResult = impro.parse(improQueryString);
        ...
        if (targetContentType === 'image/png') {
          ...
          pipeline._attach(new JpegTran(['-optimize']));
        } else {
        ...
        if (operations.length > 0 || automaticFiltersApplied) {
          // force the operations to be enumerated an an output type generated
          pipeline.flush();
