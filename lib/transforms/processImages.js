...
module.exports = (queryObj, options) => {
    ...
    return async function processImages(assetGraph) {
        ...
        await Promise.map(assetGraph.findAssets(_.extend({isImage: true, isInline: false}, queryObj)), async imageAsset => {
            ...
            const newUrl = imageAsset.url.replace(/\/([^\.\/]*)([^\?\/]*)\?[^#]*/, '/$1' + (usedQueryString ? '.' + usedQueryString.replace(/['`=<>]/g, '') : '') + '$2' + (leftOverQueryString ? '?' + leftOverQueryString : ''));
                    ...
                    filters[filters.length - 1]
                        ...
                        .on('end', () => {
                            ...
                            if (targetContentType && targetContentType !== imageAsset.contentType) {
                                const processedImageAsset = assetGraph.addAsset({
                                    type: assetGraph.typeByContentType[targetContentType] || 'Image',
                                    contentType: targetContentType,
                                    devicePixelRatio: dpr,
                                    rawSrc
                                });
                                for (const incomingRelation of imageAsset.incomingRelations) {
                                    incomingRelation.to = processedImageAsset;
                                }
                                assetGraph.removeAsset(imageAsset);
