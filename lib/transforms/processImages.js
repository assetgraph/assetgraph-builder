...
module.exports = (queryObj, options) => {
    ...
    return async function processImages(assetGraph) {
        ...
        await Promise.map(Object.keys(isAvailableByBinaryName), async binaryName => {
            if (binaryName === 'jpegtran' || binaryName === 'optipng' || binaryName === 'pngcrush' || binaryName === 'pngquant') {
                try {
                    ...
                    assetGraph.warn(new Error('processImages: ' + binaryName + ' not installed. Install it to get smaller ' + (binaryName === 'jpegtran' ? 'jpgs' : 'pngs')));
                    isAvailableByBinaryName[binaryName] = false;
                }
            } else {
                try {
                    await Promise.fromNode(
                        cb => childProcess.execFile(binaryName, cb)
                    );
                } catch (err) {
                    if (err.code === 127 || err.code === 'ENOENT') {
                        if (binaryName !== 'gm' && binaryName !== 'inkscape') {
                            assetGraph.warn(new Error('processImages: ' + binaryName + ' not installed. Install it to get smaller pngs'));
                        }
                        isAvailableByBinaryName[binaryName] = false;
                    }
                }
        ...
        function makeUrlUnique(originalUrl, safeUrl) {
            let nextSuffixToTry = 1;
            let url = originalUrl;
            while (url !== safeUrl && assetGraph._urlIndex[url]) {
                const urlObj = new URL(originalUrl);
                urlObj.pathname = urlObj.pathname.replace(/([^/]*?)([\.@][^/]*)?$/, ($0, $1, $2) => {
                    return $1 + '-' + nextSuffixToTry + ($2 || '');
                });
                url = urlObj.toString();
                nextSuffixToTry += 1;
            }
        ...
        await Promise.map(assetGraph.findAssets(_.extend({isImage: true, isInline: false}, queryObj)), async imageAsset => {
            const filters = [];
            ...
            if (matchQueryString) {
                const filterInfosAndTargetContentType = getFilterInfosAndTargetContentTypeFromQueryString(matchQueryString[1], {
                ...
                const leftOverQueryStringFragments = filterInfosAndTargetContentType.leftOverQueryStringFragments.filter(keyValuePair => {
                ...
                if (filterInfosAndTargetContentType.filterInfos.length > 0) {
                    ...
                    operationNames.push(...filterInfosAndTargetContentType.operationNames);
            ...
            let newUrl = imageAsset.url.replace(/\/([^\.\/]*)([^\?\/]*)\?[^#]*/, '/$1' + (usedQueryString ? '.' + usedQueryString.replace(/['`=<>]/g, '') : '') + '$2' + (leftOverQueryString ? '?' + leftOverQueryString : ''));
            if (filters.length > 0) {
                await new Promise((resolve, reject) => {
                    for (const [i, filter] of filters.entries()) {
                        ...
                        filter.on('error', err => {
                            ...
                            assetGraph[hasNonAutoLosslessFilters ? 'error' : 'warn'](err);
                            reject(err);
                        });
                    }
                    const chunks = [];
                        ...
                        .on('end', () => {
                            try {
                                ...
                                if (targetContentType && targetContentType !== imageAsset.contentType) {
                                    ...
                                    processedImageAsset.url = makeUrlUnique(newUrl, imageAsset.url);
                                    processedImageAsset.extension = processedImageAsset.defaultExtension;
                                } else {
                                    imageAsset.rawSrc = rawSrc;
                                    imageAsset.url = makeUrlUnique(newUrl, imageAsset.url);
