...
function liveCssClientCode() {
    if (!Array.prototype.map) {
        Array.prototype.map = function (lambda, scope) {
            var result = [], i = 0;
            for (; i < this.length; i += 1) {
                result.push(lambda.call(scope || window, this[i], i, this));
            }
            return result;
        };
    }
    ...
    socketIoScriptElement.onload = function () {
        function findCssIncludes() {
            ...
            for (i = 0 ; i < linkTags.length ; i += 1) {
                if (/\bstylesheet\b/i.test(linkTags[i].getAttribute('rel'))) {
                    cssIncludes.push({type: 'link', href: linkTags[i].getAttribute('href'), node: linkTags[i]});
                }
            }
        ...
        socket.on('connect', function () {
            socket.emit('watch', findCssIncludes().map(function (cssInclude) {return cssInclude.href;}), location.href);
