    ...
    return function autoprefixer(assetGraph) {
        ...
        if (cssAssets.length > 0) {
            try {
                ...
                assetGraph.emit('info', new Error('autoprefixerTransform: Found ' + cssAssets.length + ' css asset(s), but no autoprefixer module is available. Please use npm to install autoprefixer in your project so the autoprefixer transform can require it.'));
                return;
            }
