    ...
    return function injectOneBootstrapper(assetGraph) {
        assetGraph.findAssets(queryObj).forEach(function (initialAsset) {
            if (initialAsset.type !== 'Html' && initialAsset.type !== 'JavaScript') {
                throw new Error('transforms.injectOneBootstrapper: queryObj must only match Html and JavaScript assets, but got ' + initialAsset);
            }
            var statementAsts = uglifyAst.getFunctionBodyAst(bootstrapCode),
                allLanguageKeys = i18nTools.extractAllReachableKeys(assetGraph, initialAsset);
            ...
            var bootstrapAsset = new assetGraph.constructor.assets.JavaScript({parseTree: ['toplevel', statementAsts]});
