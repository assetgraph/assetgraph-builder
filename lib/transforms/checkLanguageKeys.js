...
module.exports = function (options) {
    ...
    return function checkLanguageKeys(assetGraph) {
        ...
        supportedLocaleIds.forEach(function (localeId) {
            allKeysByLocaleId[localeId] = i18nTools.extractAllKeysForLocale(assetGraph, localeId);
        });
        ...
        assetGraph.findAssets({type: ['Html', 'Svg', 'Htc', 'JavaScript'], isLoaded: true}).forEach(function (asset) {
            ...
            i18nTools.eachOccurrenceInAsset(asset, function (options) {
                ...
                supportedLocaleIds.forEach(function (localeId) {
                    ...
                    var value = allKeysByLocaleId[localeId][key];
        ...
        function getAssetUrlStringForKey(key) {
            ...
            return _.uniq(occurrences.map(function (occurrence) {
        ...
        Object.keys(whitespaceWarningsByKey).forEach(function (whitespaceWarningKey) {
            emitEvent('whitespace', new Error('Language key ' + whitespaceWarningKey + ' has leading or trailing whitespace: ' + util.inspect(_.map(whitespaceWarningsByKey[whitespaceWarningKey], 'value').join(', ')) + ' (used in ' + getAssetUrlStringForKey(whitespaceWarningKey) + ')'));
        });
