    ...
    return function checkLanguageKeys(assetGraph) {
        ...
        assetGraph.findAssets({type: 'I18n', isLoaded: true}).forEach(function (i18nAsset) {
            ...
            Object.keys(i18nAsset.parseTree).forEach(function (key) {
                ...
                if (Object.keys(valueByLocaleId).every(function (localeId) {
                    var value = valueByLocaleId[localeId];
                    ...
                    Object.keys(valueByLocaleId).forEach(function (localeId) {
                        var actualKeys = Object.keys(valueByLocaleId[localeId]).sort();
                        var expectedKeys = require('plurals-cldr').forms(localeId).sort();
                        var equal = actualKeys.length === expectedKeys.length;
                        if (equal) {
                        ...
                        if (!equal) {
                            emitEvent('warn', new Error(localeId + ' is missing the somethingsomething in ' + key));
                        }
                    });
                }
            });
