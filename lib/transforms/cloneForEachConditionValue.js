...
module.exports = function (queryObj, options) {
    ...
    return function cloneForEachConditionValue(assetGraph) {
        ...
        for (const splitCondition of splitConditions) {
            ...
            for (const htmlAsset of assetGraph.findAssets({type: 'Html', isInline: false, ...queryObj})) {
                for (const relation of assetGraph.findRelations({from: htmlAsset})) {
                    const dataSystemJsConditions = relation.node && assetGraphConditions.parse(relation.node);
                ...
                let conditionValues = (options.conditions && options.conditions[splitCondition]) || Object.keys(isSeenByConditionValue);
                ...
                if (conditionValues.length > 0) {
                    for (const conditionValue of conditionValues) {
                        ...
                        if (documentElement) {
                            const htmlSystemJsConditionals = assetGraphConditions.parse(documentElement) || {};
                        ...
                        const targetUrl = htmlAsset.url.replace(/(\.\w+)?$/, '.' + conditionValue + '$1');
                        clonedHtmlAsset.url = targetUrl;

                        for (const relation of assetGraph.findRelations({from: clonedHtmlAsset})) {
                            const dataSystemJsConditions = relation.node && assetGraphConditions.parse(relation.node);
                            if (dataSystemJsConditions) {
                                let key = splitCondition + '.js|default';
                                let value = dataSystemJsConditions[key];
