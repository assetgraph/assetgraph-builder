...
function run(commandAndArgs) {
    ...
    var command = commandAndArgs.map(function (arg) {
...
expect.addAssertion('<string|array> [when] run as a shell command <assertion?>', function (expect, subject) {
...
describe('buildProduction', function () {
    it('should honor --browsers "IE 8" when serializing Javascript', function () {
        ...
        return expect([
            ...
            '--root', pathModule.resolve(__dirname, '..', '..', 'testdata', 'bin', 'buildProduction', 'javaScriptWithInternetExplorer8'),
            '--outroot', tmpDir,
            '--browsers', 'IE 8',
            pathModule.resolve(__dirname, '..', '..', 'testdata', 'bin', 'buildProduction', 'javaScriptWithInternetExplorer8', 'index.html')
